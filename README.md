...
Для публикации приложения на сервере необходимо изменить в коде control-start.go адрес localhost на 0.0.0.0, после чего перекомпилировать файл.
Приложение будет доступно по IP-адресу сервера и указанному порту.
